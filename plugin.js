...
// All the markup and css stuff goes here
var canvasHtml = '\
<div id="helpmechoose_overlay" \
style="left: 0pt; right: 0pt; top: 0pt; bottom: 0pt; position: fixed; \
background: none repeat scroll 0% 0% white; z-index:99999999999 !important;"> \
</div>\
';


var overlayHtml = '\
<div id="helpmechoose_canvas" \
style="left:0;right:0;top:0;position:absolute;color:#333333;\
z-index:99999999999!important;font-family:Arial,Helvetica,sans-serif;font-size:12px;">\
    <div style="height:90px;border-bottom:1px solid #ccc; width: 100%; z-index: 9999; \
    background-color:#f6f6f6;left:0; position: fixed;"> \
        <div id="closeCanvas" style="font-size: 12px;text-align: center;cursor:pointer; padding:4px; background-color:rgba(226, 33, 9, 0.93);color:#fff;"> Close </div> \
        <div> \
            <div style="float:left;margin-left: 25px; padding: 20px 0;">\
                <span id="selectAll" style="color:#524D4D;font-weight: bold;cursor: pointer;">Select All</span> \
                | \
                <span id="unselectAll" style="color:#524D4D;font-weight: bold;cursor: pointer;">Unselect All</span>\
            ...
            <div id="logo" style="float:center;cursor:pointer;top: 26px;left: 45%;position: fixed;margin-bottom: 0;margin-left: -66px;margin-top: 0;z-index: 6000;height: auto;width: auto;">\
                <img style="width:182px;height:65px;" src="http://localhost/hmc/pluginassets/logo_new.png" /> \
            </div> \
            <div id="exportJSON" style="float:right;cursor:pointer;margin-right: 18px;line-height:65px;">\
                <img style="width:65px;" src="http://localhost/hmc/pluginassets/next.png" /> \
            </div> \
            <div style="clear:both;"></div>\
        </div> \
    </div> \
    <div style="margin-top:120px;"></div>\
    <div id="allImageContainer" style="padding: 0 25px; width: 1140px; margin: 0 auto;"></div>\
</div>\
';
...
action="http://localhost/hmc/import/"> \
    <input type="hidden" name="html" id="htmlData" value=""> \
</form> \
';

var getimageContainerHtmlPinterest = function(src, desc, pinId, height, width) {
    var content = '\
    <div class="imageContainer" \
    style="cursor:pointer;float:left;width:250px;\
    margin:15px;background:#fff;position:relative;border-radius:4px;border:1px solid #ddd; box-shadow: 4px 4px 8px #CFCFCF; -moz-box-shadow: 4px 4px 8px #CFCFCF;"> \
        <div style="width:250px;" align="center"> \
            <div class="checkMark" style="display:none;width:25px;height:25px;position:absolute;top:5px;left:5px;">\
                <!-- <img height="25" width="25" src="http://localhost/hmc/pluginassets/tick2.png">-->\
                <img height="25" width="25" src="http://localhost/hmc/pluginassets/tick2.png">\
            </div>\
            <div>\
        ...
        <div class="dimensions" style="height:20px;\
        background-color:#fff;padding:3px;text-align:center;">' + width + ' x ' + height + '\
        </div>\
        <div class="caption" style="height:50px;overflow:hidden;\
        background-color:#f6f6f6;border-bottom:1px solid #ccc;border-radius: 0 0 4px 4px;">\
            <div class="descriptionText" style="text-align: center; padding:10px 15px;">' + desc + '</div>\
        </div>\
        <div class="pinId" style="display:none;">' + pinId + '</div> \
    </div>\
    ';
...
var getImageContainerHtml = function(src, desc, height, width) {
    var content = '\
    <div class="imageContainer" \
    style="cursor:pointer;float:left;width:250px;\
    margin:15px;background:#fff;position:relative;border-radius:4px;border:1px solid #ddd; box-shadow: 4px 4px 8px #CFCFCF; -moz-box-shadow: 4px 4px 8px #CFCFCF;"> \
        ...
        <div class="caption" style="height:50px;overflow:hidden;\
        background-color:#f6f6f6;border-bottom:1px solid #ccc;border-radius: 0 0 4px 4px;">\
            <div class="descriptionText" style="padding: 10px 15px; text-align: center;">' + desc + '</div>\
        </div>\
    </div>\
    ';
    return content;
...
//anything left from previous usage
function cleanup() {
    var $ = jQuery;
    $("#helpmechoose_canvas").remove();
    $("#helpmechoose_overlay").remove();
}

/* Called once all the dependencies are loaded */
function init() {
    ...
    // show the basic skeleton first
    $body.append(canvasHtml);
    $body.append(overlayHtml);

    var $canvas = $("#helpmechoose_canvas");
    var $overlay = $("#helpmechoose_overlay");

    // build image content
    var r = '';

    if (window.location.href.indexOf('pinterest.com') != -1) { //Pinterest special
        $.each($imgContainers, function(e, t) {
            var n = t;
            var img = new Image;
            img.src = (t.getAttribute ? t.getAttribute("src") : false) || t.src;
            var desc = (t.getAttribute ? t.getAttribute("alt") : false) || t.alt;
            var isPin = $(n).hasClass('pinImg');
            var pinId = $(n).closest('a').attr('href');

            if (desc == "") {
                desc = n.alt;
            }

            if ($(n).hasClass('pinImage')) {
                var locationHref = window.location.href;
                pinId = locationHref.substring(locationHref.indexOf('/pin/'), locationHref.length);
                isPin = true;
            }

            if (isPin && pinId && (img.width > 90 && img.height > 90 || t.width > 90 && t.height > 90)) {
                pinId = pinId.substring(5, pinId.length - 1);
                r += getimageContainerHtmlPinterest(n.src, desc, pinId, img.height, img.width);
            }
        });
    } else { //For all other sites
        $.each($imgContainers, function(e, t) {
            var n = t;
            //if image has rich pin data attributes - then use them
            var $n = $(n);
            if ($n.attr('data-pin-description') != undefined && $n.attr('data-pin-media') != undefined) {
                var img = new Image;
                img.src = $n.attr('data-pin-media');
                r += getImageContainerHtml($n.attr('data-pin-media'), $n.attr('data-pin-description'), img.height, img.width);
            } else {
                var img = new Image;
                img.src = (t.getAttribute ? t.getAttribute("src") : false) || t.src;
                if (img.width > 90 && img.height > 90 || t.width > 90 && t.height > 90) {
                    r += getImageContainerHtml(n.src, n.alt, img.height, img.width);
                }
            }
    ...
    $("#allImageContainer").html(r);
    $canvas.append(hiddenForm);

    var $allImageContainer = $("#allImageContainer");

    try {
        $allImageContainer.masonry({
            columnWidth: 0,
...
//attach handlers for different actions we have

var attachHandlers = function() {
    var $ = jQuery;
    var $doc = $(document);

    $doc.delegate("#closeCanvas", "click", function() {
        cleanup();
    });

    $doc.delegate("#unselectAll", "click", function() {
        var $imgContainers = $(".imageContainer");
        $.each($imgContainers, function(e, image) {
            $(image).find(".checkMark").hide();
            $(image).removeClass("Pinterest_selected")
        });
    });

    $(".imageContainer").click(function() {
        var e = $(this);
        var t = e.find(".checkMark");
        if (t.css("display") === "block") {
            t.hide();
            e.removeClass("Pinterest_selected")
        } else {
            t.show();
            e.addClass("Pinterest_selected")
        }
    });

    $doc.delegate("#selectAll", "click", function() {
        var e = $(".imageContainer");
        $.each(e, function(e, t) {
            var n = $(t);
            n.find(".checkMark").show();
            n.addClass("Pinterest_selected")
        })
    });

    $("#exportJSON").click(function() {
        var e = $(".Pinterest_selected");
        if (e.length == 0) {
            alert('Please select images. Click on the image to select');
            return;
        }
        var t = '{"source": "' + window.location.href + '", "pins":[';
        var n = 0;
        ...
        $.each(e, function(e, i) {
            ...
            t += '{"image": "' + escape($(i).find(".actualImage").attr("src")) + '",';
            t += '"source": "' + escape(r) + '",';

            if (window.location.href.indexOf('pinterest.com') != -1) { //Pinterest special
                t += '"pinid": "' + escape($(i).find(".pinId").text()) + '",';
            }

            t += '"caption": "' + escape($(i).find(".caption .descriptionText").text()) + '"}';
            n++
        });
        t += "]}";
        $("#htmlData").val(t);
